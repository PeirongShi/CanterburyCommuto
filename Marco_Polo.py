...
def overlap_rec(csv_file: str, api_key: str, output_csv: str = "outputRec.csv") -> list:
    """
    ...
    for row in data:
        ...
        rectangles_a = create_segment_rectangles(a_segment_distances["before_segments"] + a_segment_distances["after_segments"])
        rectangles_b = create_segment_rectangles(b_segment_distances["before_segments"] + b_segment_distances["after_segments"])
...
def only_overlap_rec(csv_file: str, api_key: str, output_csv: str = "outputRec_only_overlap.csv") -> list:
    """
    Processes routes from a CSV file, computes time and distance travelled before, during,
    and after overlaps, and writes results to a CSV file.
...
def Overlap_Function(csv_file, api_key, width=100):
    """
    ...
    Args:
        csv_file (str): Path to the input CSV file containing route data.
            The CSV should include columns like:
                ...
                - Origin of B
                - Destination of B
        api_key (str): Google API key for fetching route data and calculating distances/times.
        width (int, optional): Width (in meters) for creating rectangles around route segments
            for overlap approximation. Defaults to 100.
    ...
    if option.lower() == 'yes':
        ...
        if call.lower() == 'yes':
            overlap_rec(csv_file, api_key, output_csv="outputRec.csv")
        elif call.lower() == 'no':
            only_overlap_rec(csv_file, api_key, output_csv="outputRec_only_overlap.csv")
